...
var kitaichi = {'kitaichi01':'3.72%+ (7-28-6-19-0,28-10-3-0-10)','kitaichi02':'3.68% (7-28-6-19-0)','kitaichi03':'3.68% (7-28-6-19-0)(S.teller同色)','kitaichi04':'3.68% (8-16-7-30-0)(S.teller同色)','kitaichi05':'3.61%+ (9-20-9-0-65)(S.teller同色)','kitaichi06':'3.50%+ (8-14-9-25-0,14-14-3-0-10)','kitaichi07':'3.49%+ (11-26-15-0-40)','kitaichi08':'3.40% (8-14-9-25-0)','kitaichi09':'3.14% (8-19-7-22-0)','kitaichi10':'3.07% (7-30-6-15-0)','kitaichi11':'3.05%+ (7-28-6-12-10)','kitaichi12':'2.75%+ (6-24-5-12-12)','kitaichi13':'2.52% (7-28-6-13-0)','kitaichi14':'2.50%+ (9-20-9-0-45)','kitaichi15':'2.50% (4-35-3-12-0)','kitaichi16':'2.46% (8-16-7-20-0)','kitaichi17':'2.45% (7-30-6-12-0)','kitaichi18':'2.34%+ (9-18-7-12-19)','kitaichi19':'2.33% (8-16-7-19-0)(S.teller他色)','kitaichi20':'2.32% (7-28-6-12-0)','kitaichi21':'2.28%+ (9-20-9-0-41)(S.teller他色)','kitaichi22':'2.22% (8-18-5-22-0)','kitaichi23':'2.12% (8-19-5-20-0)','kitaichi24':'2.04% (9-23-8-12-0)','kitaichi25':'2.01%+ (8-17-5-17-10)','kitaichi26':'1.96% (9-22-8-12-0)','kitaichi27':'1.86% (8-21-7-12-0)','kitaichi28':'1.73% (8-16-5-19-0)','kitaichi29':'1.73%+ (9-18-9-0-34)','kitaichi30':'1.63% (8-17-5-17-0)','kitaichi31':'1.43% (10-15-9-12-0)','kitaichi32':'1.09% (11-11-10-12-0)','kitaichi33':'0.95% (7-10-6-12-0)','kitaichi34':'0.78%+ (9-18-7-0-19)','kitaichi35':'0.69%+ (4-35-3-0-10)','kitaichi36':'0.68%+ (7-30-6-0-10)','kitaichi37':'0.67%+ (6-24-5-0-12)','kitaichi38':'0.57% (12-6-10-12-0)','kitaichi39':'0.57%+ (9-23-8-0-10)','kitaichi40':'0.55%+ (9-22-8-0-10)','kitaichi41':'0.52%+ (8-21-7-0-10)','kitaichi42':'0.40%+ (10-15-9-0-10)','kitaichi43':'0.30%+ (11-11-10-0-10)','kitaichi44':'0.26%+ (7-10-6-0-10)','kitaichi45':'0.16%+ (12-6-10-0-10)','kitaichi46':'Fleek (3-20-?-14-0)(未対応)','kitaichi47':'0.00% (0-0-0-0-0)'}
...
function output(){
    ...
    /*変数取得*/
    let Fsougou = document.getElementById("setFsougou").value;
    let Gsougou = document.getElementById("setGsougou").value;
    ...
    /*配列設定*/
    let baiarr = new Array(5);
    let frontarr = new Array(5);
    sigma = new Array(7);
    for(let i = 0 ; i < 5 ; i++){
        frontarr[i] = new Array(5);
        for(let j = 0 ; j < 5 ; j++){
            frontarr[i][j] = 0;
        }
    }
    let songnotessec = new Array(noteslength);
    for(let i = 0 ; i < noteslength ; i++){
        songnotessec[i] = new Array(2);
        for(let j = 0 ; j < 2 ; j++){
            songnotessec[i][j] = 0;
        }
    }
    let simarr = new Array(simlength);
    for(let i = 0 ; i < simlength ; i++){
        simarr[i] = new Array(21);
        for(let j = 0 ; j < 21 ; j++){
            simarr[i][j] = 0;
        }
    }
    let songarr = new Array(noteslength);
    for(let i = 0 ; i < noteslength ; i++){
        songarr[i] = new Array(4);
        for(let j = 0 ; j < 4 ; j++){
            songarr[i][j] = 0;
        }
    }
    scorearr = new Array(scoresimloop);
    for(let i = 0 ; i < scoresimloop ; i++){
        scorearr[i] = new Array(3);
        for(let j = 0 ; j < 3 ; j++){
            scorearr[i][j] = 0;
        }
    }
    sigmaresult = new Array(scoresimloop);
    sigmaresultcount = new Array(scoresimloop);
    ...
    for(let i = 0 ; i < noteslength ; i++){
        for(let j = 0 ; j < songlist.length ; j++){
            if(songlist[j].category == song[document.getElementById("songselect").value]){
    ...
    for(let k = 0 ; k < scoresimloop ; k++){
        ...
        for(let i = 0 ; i < noteslength ; i++){
            songarr[i][0] = i+1;
            songarr[i][1] = (simarr[songnotessec[i][1]-1][4]*frontarr[0][4] + simarr[songnotessec[i][1]-1][8]*frontarr[1][4] + simarr[songnotessec[i][1]-1][12]*frontarr[2][4] + simarr[songnotessec[i][1]-1][16]*frontarr[3][4] + simarr[songnotessec[i][1]-1][20]*frontarr[4][4])/100;
            songarr[i][2] = (simarr[songnotessec[i][1]-1][4]*frontarr[0][3] + simarr[songnotessec[i][1]-1][8]*frontarr[1][3] + simarr[songnotessec[i][1]-1][12]*frontarr[2][3] + simarr[songnotessec[i][1]-1][16]*frontarr[3][3] + simarr[songnotessec[i][1]-1][20]*frontarr[4][3])/100;
            if(i+1 >= FSnotes && i+1 <= FFnotes){
                songarr[i][3] = Math.floor(basescore*1.2*(1+(i+1)/noteslength*(1+songarr[i][1]))*(1+songarr[i][2]));
            }
            else{
                songarr[i][3] = Math.floor(basescore*(1+(i+1)/noteslength*(1+songarr[i][1]))*(1+songarr[i][2]));
            }
            /*1ループ分のスコア合計*/
    ...
    Graphdata_sub = {};
    for (let i = 0 ; i < sigmaresultcount.length ; i++) {
        let elm = sigmaresultcount[i];
        Graphdata_sub[elm] = (Graphdata_sub[elm] || 0) + 1;
    }
    /*グラフ元連想配列作成*/
    Graphdata = [];
    for(let i = 0 ; i < Object.keys(Graphdata_sub).length ; i++){
        let data_x = {};
        data_x.x = parseInt(Object.keys(Graphdata_sub)[i]);
        data_x.y = Object.values(Graphdata_sub)[i]/scoresimloop;
        /*data_x.px = (i+1)/(Object.keys(Graphdata_sub).length+1)*100;*/
    ...
    console.log(sigmaresultcount);
    console.log(Object.keys(Graphdata_sub).length);
    console.log(Object.values(Graphdata_sub));
